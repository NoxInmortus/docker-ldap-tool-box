...
\$advanced_search_criteria = array('firstname', 'lastname', 'mail', 'title', 'postalcode', 'organization', 'businesscategory', 'employeetype', 'created', 'modified');
\$advanded_search_display_search_objects = true;

# Results display
\$results_display_mode = "boxes";  // boxes or table
\$search_result_items = array('mail', 'organization', 'postalcode');
...
\$display_edit_link = "${LDAP_EDIT_LINK:-http://ldapadmin.example.com/?dn=\{dn\}}";
...
\$csv_items = array('firstname', 'lastname', 'mail', 'phone', 'postalcode', 'organization');

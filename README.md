...
Builds always use the master branch of {`white-pages`,`service-desk`,`self-service-password`}.
...
|LDAP_USE_PPOLICY_CONTROL|`false`|Enable LDAP password policy control|
|LDAP_AD_MODE|`false`|Active Directory mode|
|LDAP_HASH|`auto`|Hash mechanism for password|
|LDAP_SHOW_EXTENDED_ERROR|`false`|Show extended error message returned by LDAP directory when password is refused|
...
|LDAP_USE_CAPTCHA|`false`||
|LDAP_DEFAULT_ACTION|`change`||
|LDAP_USE_RESTAPI|`false`||
